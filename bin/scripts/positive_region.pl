...
foreach my $chr (@chroms) {
	...
	for (my $i=0; $i<$trans_sites{$chr}; $i++) {
		...
		my $z = ($count-$mu)/sqrt($mu*(1-$p));
		print FOUT1 $chr, "\t", $trans_coord{$chr}[$i], "\t", $trans_coord{$chr}[$i]+20, "\t", $z, "\n";
		#print FOUT1 $chr, "\t", $trans_coord{$chr}[$i], "\t", $trans_coord{$chr}[$i]+20, "\t", $z, "\t", $count,"\t", $trans_sites{$chr}, "\t", $length_chr{$chr},"\n";
	}
...
for (my $i=0; $i< scalar @cis_coord; $i++) {
	...
	my $mu = $p*$size_cis;
	my $z = ($front_count-$mu)/sqrt($mu*(1-$p));
	print FOUT1 $bait_chr, "\t", $cis_coord[$i], "\t", $cis_coord[$i]+20, "\t", $z, "\n";
}
