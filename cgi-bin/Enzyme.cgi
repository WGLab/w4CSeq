...
# the new website doesn't require to choose frag_method.
#my $frag_method = $q->param('frag_method');

## Note that I did some change in my website version
# change $buildver to $ref.
# change $fastqfile1 to $query1, and remove $fastqfile2

my $query1 = $q->param('query1');
my $query1_fh = $q->upload ('query1');
#my $fastqfile2 = $q->param('fastqfile2');
#my $fastqfile2_fh = $q->upload ('fastqfile2');
my $ref = $q->param('ref');
my $enz = $q->param('enz');
my $chipdata="NA";
my $unzip="NA";
...
sub prepareWorkDirectory {
	...
	#my $orig_file1 = "$WORK_DIRECTORY/$submission_id/$query1";
	#if ($query1 =~ /\.gz$/) {
	#	$orig_file1 = "$WORK_DIRECTORY/$submission_id/query1.fq";
	#}
	#if ($query1 !~ /\.gz$/) {
	...
	if($chipinput ne "") {
		my $orig_file2 = "$WORK_DIRECTORY/$submission_id/chipc.bed";

		open (FASTQ, ">$orig_file2") or confess "Error: cannot write chipc.bed file: $!";
		while (<$chipinput_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip1 ne "") {
		my $orig_file3 = "$WORK_DIRECTORY/$submission_id/chip1.bed";
		print CHIP_NAME $chip1_name,"\n";
		open (FASTQ, ">$orig_file3") or confess "Error: cannot write chip1.bed file: $!";
		while (<$chip1_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip2 ne "") {
		my $orig_file4 = "$WORK_DIRECTORY/$submission_id/chip2.bed";
		print CHIP_NAME $chip2_name,"\n";
		open (FASTQ, ">$orig_file4") or confess "Error: cannot write chip2.bed file: $!";
		while (<$chip2_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip3 ne "") {
		my $orig_file5 = "$WORK_DIRECTORY/$submission_id/chip3.bed";
		print CHIP_NAME $chip3_name,"\n";
		open (FASTQ, ">$orig_file5") or confess "Error: cannot write chip3.bed file: $!";
		while (<$chip3_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip4 ne "") {
		my $orig_file6 = "$WORK_DIRECTORY/$submission_id/chip4.bed";
		print CHIP_NAME $chip4_name,"\n";
		open (FASTQ, ">$orig_file6") or confess "Error: cannot write chip4.bed file: $!";
		while (<$chip4_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip5 ne "") {
		my $orig_file7 = "$WORK_DIRECTORY/$submission_id/chip5.bed";
		print CHIP_NAME $chip5_name,"\n";
		open (FASTQ, ">$orig_file7") or confess "Error: cannot write chip5.bed file: $!";
		while (<$chip5_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip6 ne "") {
		my $orig_file8 = "$WORK_DIRECTORY/$submission_id/chip6.bed";
		print CHIP_NAME $chip6_name,"\n";
		open (FASTQ, ">$orig_file8") or confess "Error: cannot write chip6.bed file: $!";
		while (<$chip6_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip7 ne "") {
		my $orig_file9 = "$WORK_DIRECTORY/$submission_id/chip7.bed";
		print CHIP_NAME $chip7_name,"\n";
		open (FASTQ, ">$orig_file9") or confess "Error: cannot write chip7.bed file: $!";
		while (<$chip7_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip8 ne "") {
		my $orig_file10 = "$WORK_DIRECTORY/$submission_id/chip8.bed";
		print CHIP_NAME $chip8_name,"\n";
		open (FASTQ, ">$orig_file10") or confess "Error: cannot write chip8.bed file: $!";
		while (<$chip8_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip9 ne "") {
		my $orig_file11 = "$WORK_DIRECTORY/$submission_id/chip9.bed";
		print CHIP_NAME $chip9_name,"\n";
		open (FASTQ, ">$orig_file11") or confess "Error: cannot write chip9.bed file: $!";
		while (<$chip9_fh>) {
			print FASTQ;
		}
		close (FASTQ);
        ...
        if($chip10 ne "") {
		my $orig_file12 = "$WORK_DIRECTORY/$submission_id/chip10.bed";
		print CHIP_NAME $chip10_name,"\n";
		open (FASTQ, ">$orig_file12") or confess "Error: cannot write chip10.bed file: $!";
		while (<$chip10_fh>) {
			print FASTQ;
		}
		close (FASTQ);
	...
	my $bait_chr = $q->param('bait_chr');
	my $bait_start = $q->param('bait_start');
	my $bait_end = $q->param('bait_end');
...
#        print INFO "enzyme=", join (",", @enzymelist), "\ntarget=$target\nbait_chr=$bait_chr\nbait_start=$bait_start\nbait_end=$bait_end\nsize_inter=$size_inter\nsize_intra=$size_intra\nwindow_intra=$window_intra\n";
	print INFO "enzyme=",$enz_seq{$enz},"\ntarget=$target\nbait_chr=$bait_chr\nbait_start=$bait_start\nbait_end=$bait_end\nsize_inter=$size_inter\nsize_intra=$size_intra\nwindow_intra=$window_intra\nchipdata=$chipdata\n";
        ...
        mkdir ("$HTML_DIRECTORY/done/$submission_id/$password") or confess "can not create submission $submission_id password $password folder:$!";
...
sub executeProgram {
		...
		if ($pid) {	#parent process where pid=child
			...
			open STDERR, '>&STDOUT';
			exec("$BIN_DIRECTORY/control_w4cseq_enzyme.pl $submission_id 2> $WORK_DIRECTORY/$submission_id/tempfile"); #excute control file
			exit;
		} else {
